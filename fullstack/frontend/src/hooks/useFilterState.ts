 ...
 */
export function useFilterState(defaultValues: {
  years?: string[];
  accounts?: string[];
  months?: string[];
  categories?: string[];
  ...
  const [years, setYearsState] = useState<string[]>(() => getParamAsArray('years') || defaultValues.years || []);
  const [accounts, setAccountsState] = useState<string[]>(() => getParamAsArray('accounts') || defaultValues.accounts || []);
  const [months, setMonthsState] = useState<string[]>(() => getParamAsArray('months') || defaultValues.months || []);
  const [categories, setCategoriesState] = useState<string[]>(() => getParamAsArray('categories') || defaultValues.categories || []);
  const [budgets, setBudgetsState] = useState<string[]>(() => getParamAsArray('budgets') || defaultValues.budgets || []);
  ...
  function updateSearchParam(params: URLSearchParams, key: string, values: string[]) {
