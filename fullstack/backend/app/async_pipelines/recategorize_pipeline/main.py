...
from app.async_pipelines.uploaded_file_pipeline.local_types import InProcessFile, TransactionsWrapper
from app.async_pipelines.uploaded_file_pipeline.transaction_parser import   archive_transactions_if_necessary
...
def apply_existing_transactions(in_process: InProcessFile)-> InProcessFile:
    ...
    query = in_process.session.query(Transaction).filter(Transaction.transaction_source_id == in_process.config.transaction_source_id, Transaction.user_id == in_process.user.id, ~Transaction.archived)
    return replace(in_process, transactions=TransactionsWrapper(transactions=[row for row in query]))
...
def apply_upload_config_no_create(process: InProcessFile) -> InProcessFile:
    ...
    if not config:
        # protects against having forgot to update the job config in the past
        query = process.session.query(UploadConfiguration).filter(UploadConfiguration.user_id == process.user.id).all()
