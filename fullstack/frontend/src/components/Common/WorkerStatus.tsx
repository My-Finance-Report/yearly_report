...
export function CollapsibleWorkerStatus() {
  ...
  const { data, isLoading, refetch } = useQuery<WorkerStatusOut[], Error>({
    ...
    refetchInterval: (data) =>
      data?.state.data?.some((status) => status.status === "completed")
        ? undefined
        : 5000,
  });
  ...
  return (
    <>
      <Flex
        onClick={statusDisclosure.onOpen}
        alignItems="center"
        gap={2}
        cursor="pointer"
        p={2}
            ...
            <Dialog.Content>
              ...
              <Box p={4} pl={10} flex={"column"}>
                ...
                <Box onClick={statusDisclosure.onClose} asChild>
                  <Text mt={4} textDecoration="underline">
                    <Link to="/faq">Why is this taking so long?</Link>
                  </Text>
                </Box>
              </Box>
            </Dialog.Content>
