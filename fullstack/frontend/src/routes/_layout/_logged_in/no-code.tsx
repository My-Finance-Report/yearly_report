...
import { NoCodeService,  NoCodeTool_Input,   NoCodeWidget,   PipelineEnd } from "@/client"
...
function NoCodeBuilder() {
  ...
  const [result, setResult] = useState<NoCodeWidget[] | null>(null)
            ...
            <Box key={`${node.tool}-${idx}`} borderWidth="1px" p={3} mb={2}>
              ...
              {node.parameters && (
                node.parameters.map((param, paramIdx) => (
                  <NoCodeParameter key={`${param.name}-${paramIdx}`} parameter={param} onChange={(val)=>onChange(val, paramIdx)} />
                ))
              )}
            </Box>
