...
import { FaUniversity, FaSync,  FaExclamationTriangle } from 'react-icons/fa';
...
const useSyncLogs = (accountId: number) => {
  return useQuery({
    ...
    queryFn: async () => {
      const response = await AccountsService.getAccountSyncLogs({ sourceId: accountId, limit: 1000 });
      if (!response) {
        ...
        <VStack align="start" gap={1}>
          <Flex align="center">
            <Text fontWeight="medium">Data Syncs Automatically</Text>
              <Flex align="center" ml={2} color="gray.500" cursor="help">
                ...
                <Text fontSize="xs">{formattedLocalTime} ({timeZone})</Text>
              </Flex>
          </Flex>
          <Text fontSize="sm">
            This account is connected to Plaid and transactions are synced daily.
          ...
          {lastSync.added_count !== null && (
            <Text fontSize="xs" color="gray.600">
              Last sync: {lastSync.added_count} added, {lastSync.modified_count} modified, {lastSync.removed_count} removed
            </Text>
          )}
