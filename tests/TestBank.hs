...
import Bank (TransactionType(..), BankRecord(..), aggregateByCategory, categorize, parseLine, generateHTML)
...
instance Arbitrary BankRecord where
  arbitrary = BankRecord <$> arbitrary
                         <*> arbitrary
                         <*> arbitrary
                         <*> arbitrary
                         <*> arbitrary
...
prop_aggregateByCategory_correctTotals records =
  let summary = aggregateByCategory records
      recalculatedTotals = Map.mapWithKey (\cat _ -> 
                          sum [ amount
                              | r <- records
