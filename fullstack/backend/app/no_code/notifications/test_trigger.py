...
from app.no_code.notifications import trigger
from app.no_code.notifications.trigger import (
    User, NoCodeTransaction, NewTransactionsEvent, Effect, EffectConfig,
    EffectType, EffectConditionals, check_event_against_possible_effects,
    aggregate_effects, perform_template_replacement, generate_callable_for_effect,
    propagate_effects, trigger_effects
)
...
def test_perform_template_replacement(event):
    effect = Effect(
        type=EffectType.EMAIL,
        config=EffectConfig(frequency_days=1, template="Account: {{account_name}}", subject="Subject"),
        condition=EffectConditionals.COUNT_OF_TRANSACTIONS,
        conditional_parameters={"count": 0}
    )
...
def test_trigger_effects_integrated(user, event):
    ...
    with patch("app.no_code.notifications.trigger.collect_user_effects", return_value=[effect]), \
         patch("app.no_code.notifications.trigger.send_email") as mock_send:
        trigger_effects(user, event)
        assert mock_send.called
