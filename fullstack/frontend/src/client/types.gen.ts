...
export type EffectConditionals = 'amount_over' | 'count_of_transactions' | 'unconditional';
...
export type EventType = 'new_transaction' | 'new_account_linked' | 'account_deactivated' | 'budget_threshold_exceeded' | 'daily' | 'weekly' | 'monthly';
...
export type NoCodeWidgetOut = {
    ...
    name: string;
    description: string;
    result: (Array<NoCodeTransaction> | string | number | Array<(string)> | Array<(number)> | Array<KeyValuePair> | ResultWithTrend | Array<NoCodeAggregateData> | null);
...
export type OrderableInput = {
    name: string;
    price: (number | string);
    variant_groups: Array<VariantGroupOutput_Input>;
    id?: (number | null);
};
...
export type OrderableOutput_Output = {
    name: string;
    price: string;
    variant_groups: Array<VariantGroupOutput_Output>;
    id: number;
};
