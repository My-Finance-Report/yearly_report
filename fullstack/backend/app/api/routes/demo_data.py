...
def generate_demo_data(num_categories=20, num_transactions=1000) -> DemoData:
    sources = []
    for index,bank in enumerate(["Wells Fargo Checking", "Capital One Credit Card", "Wells Fargo Savings"]):
        sources.append(TransactionSource(
            id=index + 1,
            name=bank,  
            user_id=1,
            archived=False,
    ...
    for i in range(1, num_transactions + 1):
        ...
        transactions.append(Transaction(
            ...
            kind=random.choice([TransactionKind.deposit, TransactionKind.withdrawal]),
            user_id=1,
            archived=False,
        ))
