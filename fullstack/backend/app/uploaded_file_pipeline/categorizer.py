...
def generate_categorization_prompt(categories: list[str], transactions: list[PartialTransaction]) -> Prompt:

    return [ChatMessage(role="user", content=f"""
...
def categorize_extracted_transactions(process: InProcessFile) -> InProcessFile:
    ...
    logger.info(f"Categorizing {len(process.transactions.transactions)} transactions...")
...
def insert_categorized_transactions(in_process: InProcessFile)->None:
    ...
    transactions_to_insert = [
        Transaction(
            ...
            category_id=category_lookup[t.category].id,
            date_of_transaction=datetime.strptime(t.partialTransactionDateOfTransaction, "%m/%d/%Y"),
            amount=t.partialTransactionAmount,
            transaction_source_id=in_process.file.transaction_source_id,
