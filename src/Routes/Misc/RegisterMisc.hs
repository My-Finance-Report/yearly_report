...
import HtmlGenerators.HomePage (renderHomePage, makeSimpleBanner)
...
registerMiscRoutes pool activeJobs = do
    get "/" $ do
        ...
        case user of
            ...
            Nothing -> html $ renderPage user "My Financial Report" renderLandingPage

    get "/help" $ do
        user <- getCurrentUser pool
        html $ renderPage user "Help Me" renderSupportPage
