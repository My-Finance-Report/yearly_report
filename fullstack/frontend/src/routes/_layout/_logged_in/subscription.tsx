...
import { PriceDetails, SubscriptionDetails, SubscriptionService } from '@/client';
...
export default function Subscription() {
  ...
  const handleSubscribe = async (planId: number) => {
    try {
      ...
      const response = await SubscriptionService.createCheckoutSession({ priceId: planId });
  ...
  return (
    <Container maxW="container.xl" py={8}>
      <VStack spaceX={8} align="stretch">
        <Heading as="h1" size="xl">Subscription Management</Heading>
            ...
            <CardBody>
              <SimpleGrid columns={{ base: 1, md: 3 }} spaceX={4}>
                ...
                <StatRoot>
                  ...
                  <StatValueText>{status.current_sources} / {status.max_sources}</StatValueText>
                  <StatHelpText>
                    {status.current_sources >= status.max_sources ? 
                      'Limit reached' : 
                      `${status.max_sources - status.current_sources} remaining`
                    }
                  </StatHelpText>
                </StatRoot>
                ...
                {status.current_period_end && (
                  <StatRoot>
                    ...
                    <StatValueText>{formatDate(status.current_period_end)}</StatValueText>
              ...
              {status.status === 'active' && status.tier !== 'free' && !status.cancel_at_period_end && (
                <Box mt={4}>
                  <Button 
                    colorScheme="red" 
                    variant="outline" 
        ...
        <Box>
          <Heading as="h2" size="lg" mb={4}>Available Plans</Heading>
          <SimpleGrid columns={{ base: 1, md: 3 }} spaceX={8}>
            {plans.map((plan) => (
              <Card.Root key={plan.id} borderWidth="1px" borderRadius="lg" overflow="hidden">
                <CardHeader bg={plan.tier === 'premium' ? 'blue.500' : 'gray.100'} color={plan.tier === 'premium' ? 'white' : 'black'}>
                  ...
                  <VStack spaceX={4} align="stretch">
                    <Box>
                      <Heading size="xl">
                        ...
                        <Text as="span" fontSize="md">/{plan.interval}</Text>
                      </Heading>
                    </Box>
                    ...
                    <Box>
                      ...
                      <VStack align="start" spaceX={2}>
                        <HStack>
                          ...
                          <Text>Up to {plan.max_sources} transaction sources</Text>
                        </HStack>
                        <HStack>
                          <Box color="green.500">
                            <FaCheck />
                ...
                <CardFooter>
                  ...
                  >
                    {status?.tier === plan.tier ? 'Current Plan' : 'Select Plan'}
                  </Button>
                </CardFooter>
              </Card.Root>
            ))}
