...
from app.schemas.subscription import SubscriptionDetails, SubscriptionLimits, PriceDetails, CheckoutSession
...
class StripeService:
    ...
    def get_transaction_source_count(db: Session, user_id: int) -> int:
        ...
        return db.query(TransactionSource).filter(
            TransactionSource.user_id == user_id,
            TransactionSource.archived == False
        ).count()
    ...
    @staticmethod
    def _handle_subscription_updated(db: Session, subscription_data: Dict[str, Any]) -> None:
    ...
    @staticmethod
    def _handle_subscription_deleted(db: Session, subscription_data: Dict[str, Any]) -> None:
