...
function AllActions({ setTimeframe }: { setTimeframe: (timeframe: Action['timeframe']) => void }) {
    return (
...
function ActionCard({ action, setTimeframe }: { action: Action, setTimeframe: (timeframe: Action['timeframe']) => void }) {
    return (
        <Box 
            cursor="pointer" 
            minH={100} 
        ...
        >
            <Text fontSize="lg" fontWeight="medium">{action.name}</Text>
        </Box>
    )
}
      ...
      <Table.Body>
        {orders.map((order) => (
          <Table.Row key={order.id}>
            <Table.Cell>{new Date(order.timestamp).toLocaleDateString()}</Table.Cell>
            <Table.Cell>${formatPrice(order.orderItems.reduce((sum, item) => sum + determinePrice(item), 0))}</Table.Cell>
            <Table.Cell>
